...
| Trust-boundary hardening (interim security assessment 2026-06-13) | Repo crossed into "security-sensitive local agent." Full findings + severities in `ASSUMPTIONS.md` § Security posture. **Worth now (personal):** S1 pin `OLLAMA_ORIGINS` to the extension ID (drop `chrome-extension://*`); S2 add a per-install token to the Libre server. **Release checklist (rides with the rows above):** nonce/MessageChannel for `lst-track` (S3), `chrome.storage` for settings (S4, = the row above), server resource caps (S5), drop `http://*/*` + narrow matches (S6). Do **not** build a subtitle prompt-injection detector — model is capability-free, output is text-only; tripwire is the model gaining any tool/file/shell access. | S1/S2 easy; rest folds into settings + packaging |
