...
| Compatibility probe (diagnostic) — **SHIPPED (0.5.32)** | Channel probe logs a per-site inventory of all four subtitle channels (native TextTracks / rendered DOM / network tracks / none-found = CC off or burned-in) plus a tier verdict and a non-English-source warning. Backed by `ASSUMPTIONS.md`, the ledger of silent assumptions with their evidence base — update it after every field test. Remaining: surface the verdict in the badge/panel (it's console-only today), which becomes the onboarding "Try on this site" report | Done (console); UI surfacing pending |
...
| Trust-boundary hardening (interim security assessment 2026-06-13) | Repo crossed into "security-sensitive local agent." Full findings + severities in `ASSUMPTIONS.md` § Security posture. **DONE 2026-06-13:** S1 — `OLLAMA_ORIGINS` pinned to the extension ID only (web origins were vestigial); S2 — Libre server enforces `Origin ==` extension + non-wildcard CORS (both verified end-to-end). **Release checklist (rides with the rows above):** nonce/MessageChannel for `lst-track` (S3), `chrome.storage` for settings (S4, = the row above), server resource caps (S5), drop `http://*/*` + narrow matches (S6), and the S2 **pre-shared token** (needs onboarding's paste UX). Do **not** build a subtitle prompt-injection detector — model is capability-free, output is text-only; tripwire is the model gaining any tool/file/shell access. | S1/S2 done; rest folds into settings + packaging |
...
| SonyLIV | **Prefetch expected** (fixes 0.5.30–31, field verify pending) | video.js with de-prefixed caption classes (`.text-track-wrapper` — recognized 0.5.30); exposes the full episode as a hidden native TextTrack (364 cues observed) → first user of the native-textTracks prefetch source (0.5.31): cues harvested straight off `video.textTracks`, no interception, no calibration needed. JW Player (`.jw-captions`) is the next likely family |
| Apple TV+ (web) | Hard | Heavy canvas/custom rendering in places; Safari-first audience |
| SUNNXT | **Live path** (verified 2026-06) | Shaka player; probe reported `native[none] net[none] → live path (DOM only)`. Root picker had latched the CC toggle button (`shaka-caption-button`) — 0.5.34 excludes interactive controls from root candidacy and filters CC-status labels. Subtitle delivery not network-sniffable so far → prefetch-lite candidate alongside MX |
| aha, Eros Now, Lionsgate Play | Unknown | Audit pass needed |
