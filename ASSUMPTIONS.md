...
| 5 | Episode identity == URL | **Violated & fixed** 2026-06-13, Prime: next/autoplay swaps episodes under a stable href, so the href-only `resetPrefetch` never fired and the new episode's track merged into the old cues (806→1701). Grounding diagnostic showed the track arrives ~1s before `<video>`/`currentSrc`/duration flip, so the fix is track-led: at ingest, reset when an incoming **full-file** track's storage key differs from the active one (0.6.1). Gated to full-file kinds (ttml-text/yt-json/timedtext-xml) so segmented players (HLS/DASH VTT, ttml-mp4) still append. Field-verified on Prime |
...
- Per-site field test must include **autoplay/next into a second episode on a stable-URL player** (the gap that hid the row-5 cross-episode bug until 2026-06-13): confirm a `prefetch reset` fires and cues drop to the new episode only. For segmented players, confirm same-episode fragments still append (no spurious reset).
